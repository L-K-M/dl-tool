...
| `DLTOOL_METRICS_ADDR` | listen addr | `127.0.0.1:9090` | no | infrastructure | Separate listener exposing `GET /metrics`. The literal value `off` disables metrics entirely — an empty string cannot express this, because empty means "unset" and falls back to the default. | `internal/obs/metrics.go` |
