...
| Rotation | a UI action regenerates each secret. Rotating `DLTOOL_SECRET_KEY` makes every stored `*_enc` value undecryptable, so the action re-encrypts what it can from the operator's re-entered values and otherwise clears them with a visible `secret_lost` marker (row-level `last_error` on indexers, channels and engines, a surfaced marker on the `extract_passwords` setting, one `task_events` row per nulled per-task password — not a `tasks.error_code`); rotating `ARIA2_RPC_SECRET` marks aria2 unhealthy until the container restarts. Sessions survive both — session ids are opaque rows, not signed tokens |
