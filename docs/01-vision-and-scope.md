...
> **Read this before:** [`02-requirements.md`](02-requirements.md), [`16-prior-art-and-research.md`](16-prior-art-and-research.md), T001
...
- In scope: problem statement with dated evidence, personas, the DS parity table, the five differentiators, v1 non-goals, quality goals, content/legal posture.
...
**The automation user (Sonarr/Radarr operator).** Points an *arr instance at dl-tool as a download client and never opens the UI. Their job to be done: dl-tool answers the qBittorrent WebAPI v2 subset at `/api/v2/*` or the Synology DownloadStation subset at `/webapi/*` faithfully enough that the existing client definition works unchanged — both façades default off, per [ADR-0014](decisions/0014-opt-in-qbittorrent-and-synology-compatibility-facades.md). Endpoint shapes live in [`15-compatibility-apis.md`](15-compatibility-apis.md).
...
| BT search with `.dlm` plug-ins | Uploaded `.dlm` tarballs run PHP `search.php` server-side | improved | Torznab/Newznab client plus declarative `dlsearch/v1` YAML; `.dlm` is statically analysed and converted, never executed → [ADR-0010](decisions/0010-never-execute-third-party-definition-code.md), [`07-search-and-indexers.md`](07-search-and-indexers.md) |
...
| Statistics | `SYNO.DownloadStation.Statistic` global up/down | parity | Reported in bytes/second everywhere; DS's KB/s-config vs B/s-reporting split is not copied |
| Notifications | Email + desktop, per-event checkboxes, requires DSM Control Panel SMTP | parity | Outbound notification via `DLTOOL_NOTIFY_URL` instead of a NAS-wide mail relay → [`11-config-reference.md`](11-config-reference.md) |
...
| Mobile client | DS get (Android), documented version 1.11 | parity | Responsive PWA plus the Synology façade, which existing DS clients already speak |
| NZB / Usenet | News server config, max connections, par2 repair and cleanup | deferred to v2 | The `Engine` interface must not preclude it → [ADR-0005](decisions/0005-aria2-qbittorrent-and-yt-dlp-as-the-v1-engines.md) |
...
## 4. What dl-tool owns that nothing else does

1. **One queue spanning HTTP/FTP/SFTP, BitTorrent and media-site URLs.** No maintained project unifies these: qBittorrent, Transmission, Deluge and Flood are BitTorrent-only, SABnzbd is NZB-only, and the one project that attempted the union has five stars and a README stating it is not production-ready.
...
| Non-goal | Reason |
|---|---|
| Writing a BitTorrent, NNTP/par2 or hoster-scraping engine | The engines are mature and expose stable HTTP APIs; re-implementing one is the classic failure of this product class → [ADR-0001](decisions/0001-build-a-control-plane-over-existing-download-engines.md) |
...
| Executing any third-party code | No PHP, no Python plug-ins, no scripting runtime; the entire `.dlm` ecosystem is a remote-code-execution surface → [ADR-0010](decisions/0010-never-execute-third-party-definition-code.md) |
