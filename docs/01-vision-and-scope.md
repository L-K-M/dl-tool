...
- Out of scope (lives instead in): requirement sentences and IDs → [`02-requirements.md`](02-requirements.md); components, engines and diagrams → [`03-architecture.md`](03-architecture.md); env vars → [`11-config-reference.md`](11-config-reference.md); compose services, ports and volumes → [`10-deployment-and-compose.md`](10-deployment-and-compose.md); the landscape survey and full parity matrix of other tools → [`16-prior-art-and-research.md`](16-prior-art-and-research.md).
...
**What a self-hoster is left with.** The engines DS wraps are individually excellent and expose stable HTTP APIs, but nothing composes them. No maintained project puts BitTorrent, Usenet, HTTP-hoster, ED2K and media-site URLs in one queue. No engine exposes a directory-listing API, so every one of them takes a destination as an opaque string and the folder picker DS users rely on simply does not exist in the open-source world. A composed stack of qBittorrent + SABnzbd + aria2 + Prowlarr + Unpackerr covers 13 of 14 DS features at the daemon level and none of them as one coherent experience, because the user must know which of five web UIs to open. dl-tool is that missing layer, and DS's own defects — the 2048/256 task caps, seeding auto-stop needing ratio **and** time, KB/s in configuration versus B/s in reporting — are the list of things it fixes on the way. It does not fix DS's single-tenancy: dl-tool authenticates one operator account ([ADR-0019](decisions/0019-single-account-no-ownership.md)).
...
**The Download Station migrant.** Has used Download Station for years and now wants the same screen on hardware they choose, deployed with `docker compose`. Their job to be done: the same screen, on hardware I choose — a UI whose sidebar, add-task dialog, detail tabs and settings tree sit exactly where Download Station put them, so nothing has to be relearnt → [`09-web-ui-spec.md`](09-web-ui-spec.md).
...
| Concurrent-task limits | Not exposed; parallelism is bounded only by `Process order` and the 2048/256 task caps | improved | `max_active_total` and `max_active_per_engine`, enforced by dl-tool because no engine can see another engine's queue; seeding counts toward none of them → [`06-download-engines.md`](06-download-engines.md) |
...
| 24×7 bandwidth schedule | Grid painted with three states: No Download · Default Speed · Alternative Speed | parity | Same three states, `GET /settings/schedule` and `PUT /settings/schedule` |
...
| Process order | `By date created` or `By user (one task at a time)` round robin across owners | partial | Creation order only. The by-user round robin has no meaning with one account → [ADR-0019](decisions/0019-single-account-no-ownership.md) |
| Task count caps | 2048 tasks for `admin`/`administrators`, 256 for ordinary users | improved | No hard cap. Concurrency is bounded by `max_active_total` and `max_active_per_engine`, not by a stored-task count |
| Multi-user | "Download Station is available for local DSM users only but not for domain users or LDAP users" | out of scope | dl-tool authenticates one operator account. Authentication is still mandatory with no default credentials → [ADR-0013](decisions/0013-mandatory-built-in-authentication.md), [ADR-0019](decisions/0019-single-account-no-ownership.md) |
| Destination containment | One global `Default Destination Folder` | parity | Every path — browsing, free space, `mkdir` and task destinations — is confined to `DLTOOL_DATA_ROOTS`, after symlink resolution → [`12-security-and-threat-model.md`](12-security-and-threat-model.md) |
...
1. **One queue spanning HTTP/FTP/SFTP, BitTorrent and media-site URLs**, for every task dl-tool created. dl-tool assumes exclusive control of its engines and ignores any transfer it did not create, so a task added directly on an engine never appears in the queue → [ADR-0017](decisions/0017-exclusive-control-of-engines.md), [`06-download-engines.md`](06-download-engines.md). No maintained project unifies these: qBittorrent, Transmission, Deluge and Flood are BitTorrent-only, SABnzbd is NZB-only, and the one project that attempted the union has five stars and a README stating it is not production-ready.
2. **Auto-extraction driven by the queue.** Download Station extracts `.zip`, `.tar`, `.gz`, `.tgz`, `.rar` and `.7z` on completion against a shared password list; in the self-hosted world this is Unpackerr, which is driven by an \*arr or a watch folder rather than by the queue that produced the file.
...
dl-tool ships **zero** piracy indexers — not disabled, not commented out, absent. Exactly four legitimate search engines are bundled and enabled by default: `internet-archive` (JSON), `arch-linux` (RSS), `academic-torrents` (RSS) and `linux-distributions` (a curated static list, not a scraper); their definitions and endpoints are in [`07-search-and-indexers.md`](07-search-and-indexers.md). Adding any other indexer is a deliberate, documented user action, and every imported engine starts **disabled** and displays its provenance. dl-tool is protocol-neutral infrastructure in the same sense as curl or a web browser; the README states that once, and neither the UI nor these documents moralise or add warnings. There is no telemetry and no phone-home update check, by default or otherwise.
...
| 2026-09-01 | Migration subsystem cut. The Download Station migrant's job to be done is now purely "the same screen, on hardware I choose"; removed the dry-run clause and the link to the withdrawn migration document, the "Migration from a live NAS" parity row, and the `foreign_task_policy` condition on the one-queue claim. `.torrent`/`.txt` upload, watch folders and backup/restore are unaffected. |
| 2026-09-01 | Consistency pass: §7 now describes the bundled `linux-distributions` engine as a curated static list, matching [`07-search-and-indexers.md`](07-search-and-indexers.md) §3.8, [ADR-0008](decisions/0008-torznab-first-declarative-yaml-second.md) and T105 — it is not an HTML directory-index scraper. |
| 2026-09-01 | Contradiction fix: the batch-paste parity row claimed "No 50-URL cap" while the API caps one request at 50 URIs; the row now states the real behaviour (per-request cap, dialog batches automatically). |
| 2026-09-02 | Multi-user dropped: the flatmate persona removed, the differentiator list rebuilt around auto-extraction, and the parity table restated — process order is creation order only, and destination containment is the data-root check rather than a per-user jail ([ADR-0019](decisions/0019-single-account-no-ownership.md)). |
