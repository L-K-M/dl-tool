...
| **Decisions** | [ADR-0012](../decisions/0012-single-data-mount.md), [ADR-0020](../decisions/0020-x-text-for-nfc-and-case-folding.md) |
...
[`docs/13-testing-and-verification.md` §7.1](../13-testing-and-verification.md), `go.mod`/`go.sum`
under that section's dependency carve-out — §3.2 step 3 and §3.3 rule 6 require
`golang.org/x/text`, recorded in
[ADR-0020](../decisions/0020-x-text-for-nfc-and-case-folding.md) — this task file's
...
`web/src/api/schema.d.ts` regenerated by `make gen`; `go.mod`/`go.sum` adding
`golang.org/x/text v0.41.0` as a new direct dependency — required by §3.2 step 3 and §3.3 rule 6,
with [ADR-0020](../decisions/0020-x-text-for-nfc-and-case-folding.md) supplying the record hard
rule 3 required alongside it).
