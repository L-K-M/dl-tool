...
ok  	github.com/L-K-M/dl-tool/internal/api	185.523s
ok  	github.com/L-K-M/dl-tool/internal/store	76.513s
BACKUP_OK
```
...
--- PASS: TestDryRunWritesNothing (1.26s)
--- PASS: TestImportIsTransactional (1.15s)
--- PASS: TestNewerDocumentVersionConflict (0.38s)
--- PASS: TestRestoreRefusesRunningServer (0.54s)
--- PASS: TestRestoreRejectsForeignSource (0.54s)
--- PASS: TestRestoreRefusesSchemaTooNew (0.59s)
--- PASS: TestRestoreRefusesCorrupt (0.36s)
--- PASS: TestRestoreAcceptsOlderSchema (1.02s)
--- PASS: TestRestoreFailureLeavesOriginal (0.84s)
ok  	github.com/L-K-M/dl-tool/internal/api	9.489s
```
...
Known plan-level gap, out of this task's contract: `RestoreFrom` takes the stable process lock and
`TestRestoreRefusesRunningServer` proves the `restore_server_running` refusal, but nothing yet takes
that lock on the server side — doc 17 §1.3 stage S3 (hold `flock(LOCK_EX|LOCK_NB)` for the process
lifetime, second server exits `database_locked`) has no owner task. This task's contract only specifies
the restore side attempting the flock; wiring the server half is boot-sequence work for whichever task
owns §1.3.
