...
## Acceptance criteria
- [x] `TestConditionalGetSendsBothValidators` and `TestNotModifiedAddsNoItems` pass.
- [x] `TestBackoffLadderEscalatesAndDecrements` asserts the exact seconds 60, 300, 900.
- [x] A response declaring a 17 MiB `Content-Length` fails the poll before its body is read.
- [x] `TestBodyCapRejectsOversizeFeed` passes and no partial item is stored.
- [x] `TestJitterIsDeterministicAndBounded` passes.
- [x] `POST /feeds/{id}/refresh` polls a feed whose `disabled_till` is in the future.
- [x] No `http.Client` is constructed in `internal/rss`.
...
go test -race -count=1 ./internal/rss/... ./internal/jobs/... ./internal/api/...
ok  	github.com/L-K-M/dl-tool/internal/rss	6.320s
ok  	github.com/L-K-M/dl-tool/internal/jobs	7.011s
ok  	github.com/L-K-M/dl-tool/internal/api	138.087s
POLL_OK
```
...
```
$ go test -count=1 -v -run 'TestConditionalGetSendsBothValidators|TestNotModifiedAddsNoItems|TestBackoffLadderEscalatesAndDecrements|TestBodyCapRejectsOversizeFeed|TestJitterIsDeterministicAndBounded' ./internal/rss/
--- PASS: TestConditionalGetSendsBothValidators (0.06s)
--- PASS: TestNotModifiedAddsNoItems (0.05s)
--- PASS: TestBackoffLadderEscalatesAndDecrements (0.04s)
--- PASS: TestBodyCapRejectsOversizeFeed (0.04s)
--- PASS: TestJitterIsDeterministicAndBounded (0.01s)
ok  	github.com/L-K-M/dl-tool/internal/rss	0.207s
```

The refresh acceptance criterion is proven at the poller level by
`TestRefreshPollsDisabledFeed` (a `Poll(f, true)` fetches a feed whose
`disabled_till` is an hour out and clears it on success) —
`internal/api/feeds_test.go` is outside the Files table, so the endpoint
itself is a thin `FeedByID` → `Poll(f, true)` → 200 path.
...
`api/openapi.json` and `web/src/api/schema.d.ts` are the generated files
`docs/13-testing-and-verification.md` section 7.1 makes implicit members of
the Files table of any task registering a Huma operation (the new
`refresh-feed` operation), both regenerated by `make gen`. `go.mod`/`go.sum`
are the implicit members of a task first importing an already-pinned
dependency — `github.com/robfig/cron/v3 v3.0.1`, pinned by T004 and approved
by ADR-0015 — committed exactly as `go mod tidy` produced them. The rest is
