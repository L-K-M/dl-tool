...
| **Blocks** | T008, T010, T013, T014, T020, T046, T055, T065, T068, T095, T129 |
...
| **Est. size** | 2 new source files, 1 test file, 2 generated files, 4 edited files, ~300 LOC |
...
2. [`docs/09-web-ui-spec.md` §1 Frontend stack](../09-web-ui-spec.md#1-frontend-stack) — the generator and
   formatter pins.
...
4. [`docs/13-testing-and-verification.md` §7 CI](../13-testing-and-verification.md#7-ci) — the drift gate the
...
| `web/src/api/schema.d.ts` | create (generated by `make gen`) | The formatted TypeScript contract. |
| `web/package.json` | edit | Add `openapi-typescript` as a pinned dev dependency. |
| `web/package-lock.json` | edit (generated by `npm install`) | Lock the generator version. |
| `cmd/dl-tool/main.go` | edit | Build the server in `OnStart`, shut it down in `OnStop`, add `openapi`. |
| `scripts/gen.sh` | edit | Generate and format the TypeScript schema before the lint and drift gates. |
...
line of stdout is exactly `API_OK`.
