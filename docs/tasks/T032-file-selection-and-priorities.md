...
Round 2, after the review fixes (deselect backstop for a null files array,
repeated-index rejection, listing-specific 503 detail,
RejectUnknownQueryParameters on PATCH, the json_each delete, and the added
reselect/skip-string/empty-listing/empty-selection tests):

`make lint && make test PKG=./internal/...` on the final tree:
...
ok  	github.com/L-K-M/dl-tool/internal/api	59.444s
ok  	github.com/L-K-M/dl-tool/internal/config	1.117s
ok  	github.com/L-K-M/dl-tool/internal/engine	21.333s
ok  	github.com/L-K-M/dl-tool/internal/engine/aria2	3.175s
ok  	github.com/L-K-M/dl-tool/internal/engine/qbittorrent	5.263s
ok  	github.com/L-K-M/dl-tool/internal/fsx	1.028s
ok  	github.com/L-K-M/dl-tool/internal/jobs	4.603s
ok  	github.com/L-K-M/dl-tool/internal/obs	1.180s
ok  	github.com/L-K-M/dl-tool/internal/secure	4.306s
ok  	github.com/L-K-M/dl-tool/internal/store	68.176s
...
ok  	github.com/L-K-M/dl-tool/internal/engine/qbittorrent	0.019s
--- PASS: TestDeselectSetsSkip (0.06s)
--- PASS: TestPatchFilesUnknownIndex (0.02s)
--- PASS: TestAria2FilesHaveNullPriority (0.04s)
ok  	github.com/L-K-M/dl-tool/internal/api	0.155s
```
...
Round 1 output (same commands, pre-review tree 6ec4cd8): lint clean,
`ok` for every `./internal/...` package, the five named tests PASS, scope
exactly the Files table plus the two generated standing exceptions.

Scope check on the final tree:
...
(excluding `docs` — the task file and index carry this Evidence and the
flipped rows — and relative to round 1, `api/openapi.json` and
`web/src/api/schema.d.ts` were regenerated by `make gen`; the round-2
registration tweak emits a byte-identical document, so both stay as
committed in the round-1 commit.) Exactly the Files table plus the two
generated standing exceptions of `docs/13-testing-and-verification.md`
§7.1 (`api/openapi.json`, `web/src/api/schema.d.ts`, both from `make gen`;
the two new Huma operations `list-task-files` and `patch-task-files`).
