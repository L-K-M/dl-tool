...
## Acceptance criteria
- [x] `TestDryRunReturnsEveryEvaluatedItem` asserts `evaluated == matched + rejected == len(results)`.
- [x] `TestDryRunReasonEnumIsClosed` fails on any code outside the ten of doc 08 §5.1.
- [x] `TestDryRunIsReproducibleWithIgnoreState` compares two reports with `go-cmp` and finds no diff.
- [x] `TestDryRunWritesNothing` asserts `rule_matches` and `rule_seen_episodes` row counts are unchanged.
- [x] `reason_detail` names the clause and its index, e.g. `none_of[0] = "daily"`.
...
ok  	github.com/L-K-M/dl-tool/internal/rss	10.962s
ok  	github.com/L-K-M/dl-tool/internal/api	157.393s
DRYRUN_OK
```

The acceptance-criteria and endpoint tests, from the same head under
`go test -race -count=1 -v -run 'TestDryRun|TestTestRule'`:

```
--- PASS: TestDryRunReturnsEveryEvaluatedItem (0.74s)
--- PASS: TestDryRunReasonEnumIsClosed (0.60s)
--- PASS: TestDryRunIsReproducibleWithIgnoreState (0.45s)
--- PASS: TestDryRunWritesNothing (0.38s)
--- PASS: TestDryRunLimitIsPerFeed (0.47s)
--- PASS: TestDryRunUnknownFeedIsNotFound (0.41s)
--- PASS: TestDryRunFeedSelection (0.46s)
--- PASS: TestTestRuleMalformedFilterIsRejected (0.58s)
--- PASS: TestTestRuleUnknownFeedIsNotFound (0.45s)
--- PASS: TestTestRuleReturnsEveryItem (0.53s)
```
...
`api/openapi.json` and `web/src/api/schema.d.ts` are regenerated by `make gen` —
implicitly part of the Files table under docs/13-testing-and-verification.md §7.1,
since this task registers the `POST /rules/test` Huma operation.

Notes on two contract details read strictly:

- `DryRunRequest.IgnoreState` is a plain `bool`, so the documented default of
  true is applied by the handler — `ignore_state` absent or null yields
  `IgnoreState: true` — while a direct `DryRun` call reads the value literally.
- The database-backed `State` for `ignore_state: false` lives in `dryrun.go` as
  the unexported `dryRunState`, read-only over `rule_matches` and
  `rule_seen_episodes`; the contract only named `StatelessState`. T071 may grow
  its own state in `grab.go` when commits arrive.
