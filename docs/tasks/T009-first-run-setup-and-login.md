...
| `api/openapi.json` | generated | Regenerated by `make gen` because this task adds Huma operations (docs/13-testing-and-verification.md §7.1). |
| `web/src/api/schema.d.ts` | generated | Regenerated by `make gen` because this task adds Huma operations (docs/13-testing-and-verification.md §7.1). |
...
- Review round (PR #52) added: PHC parameter ceilings before the argon2 derivation and `needsRehash`
  gated on a verified password; throttle maps sweep their expired entries and drop empty source
  buckets; setup serialized behind a mutex with a concurrent-calls test; failed setup attempts
  consume the source budget only, so they cannot pre-load the login ladder for an
  attacker-chosen username; `Cache-Control: no-store` on the three envelope responses;
  `format: date-time` on the wire timestamps; the document now declares the cookie and bearer
  security schemes and per-operation security; a zero `DLTOOL_SESSION_TTL` fails construction
  loudly; bearer logout documented as non-revoking (token revocation is T084's).
