...
| [T072](T072-rss-feeds-screen.md) | Build the RSS feeds and items screen | T014, T040, T044, T052, T065, T066, T068 | todo |
...
| The `auto_download` member of `POST`/`PATCH /feeds` and the `auto:<feed_id>` rule lifecycle it drives | [FR-070](../02-requirements.md#fr-070-manage-feeds-and-refresh-on-demand) | A valid `definition_json` needs T068's `RuleDoc` and the `rules` store, so T065 ships the feed writes without the member: it is absent from both schemas and a sender gets `422` under `additionalProperties: false`, never a silent no-op. T068 adds the member and the create/delete lifecycle, and T072's dependency edge keeps the add-dialog checkbox behind it. | T068 |
...
| T072 | Build the RSS feeds and items screen | T014, T040, T044, T052, T065, T066, T068 | no | todo | [T072](T072-rss-feeds-screen.md) |
