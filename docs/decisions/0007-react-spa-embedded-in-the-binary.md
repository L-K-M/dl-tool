...
The hard part of the screen is not the styling. It is a virtualized, column-resizable, column-reorderable
table of up to several thousand rows, multi-selectable, updating at 1 Hz from
[ADR-0006](0006-sse-with-rid-deltas.md), with a detail pane bound to the current row.

## Decision Drivers

- One container and one artefact. A second web server means a second image, a second port and CORS.
- The client's API types are generated from `api/openapi.json` ([ADR-0003](0003-chi-huma-code-first-openapi.md)),
  so the UI stack must be TypeScript.
- The grid and virtualisation libraries the screen needs target React first.
- Deployment behind a reverse proxy at a sub-path must work without rebuilding the image.
- The implementing agent is a weaker model, so corpus size is a real selection criterion.
...
- Good, because `docker compose up -d` yields one image serving the API and the UI on one port, with no
  CORS configuration and nothing to keep version-matched.
- Good, because `//go:embed dist` fails to compile when the pattern matches no files, so a binary can never
  ship without its UI.
- Good, because a wrong endpoint path or field name in the SPA is a TypeScript compile error rather than a
  runtime 404.
...
- Bad, because there is no no-JavaScript fallback. Accepted.
- Neutral, because the PWA rides on the same bundle — manifest, maskable icons, `standalone` display and a
  ...
  and [`../09-web-ui-spec.md`](../09-web-ui-spec.md) says so.
...
Expected: exit 0. `make build` runs the web build first and then `go build`, which fails outright if
...
that a hashed asset path returns the embedded asset, and that an unknown `/api/v1/...` path returns a
...
- Good, because one binary is one artefact to build, sign, scan and release.
- Good, because the API and the UI cannot drift in version — they are the same file.
...
- Bad, because a virtualized, resizable, reorderable multi-thousand-row grid with 1 Hz live updates is
